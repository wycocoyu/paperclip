...
| `PAPERCLIP_OV_SYNC_BIN` | (unset) | Absolute path (or `~`-prefixed) of the OpenViking `ov` CLI the server calls after a Team Rules note or Team Wiki page revision commits (MUL-559 step 4). Must be set together with `PAPERCLIP_OV_SYNC_COMPANY_ID` — both or neither; half a configuration refuses to boot. Both unset = fan-out disabled. The `personal` wiki space is never pushed: it mirrors each terminal's own CLAUDE.md / AGENTS.md, which that terminal already loads. |
| `PAPERCLIP_OV_SYNC_COMPANY_ID` | (unset) | UUID of the company whose Team Rules and Team Wiki fan out to OpenViking. One company only: the OpenViking rules/wiki URIs carry no company segment, so a second company would overwrite the first. Terminal fan-out failures surface at `GET /api/companies/:companyId/team-docs/fanout-failures`. |
